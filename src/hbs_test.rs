...
#[test]
fn test_hbs() -> Result<(), Box<dyn Error>> {
    // std::io::Result
    env_logger::init();
    let  current_dir:String= String::new();//  std::env::current_dir().unwrap_or_default().clone().;
 
//    let current_dir= match dir.to_str() {Some(str)=>str.to_string(),None=>String::new()};
   println!("{}",current_dir);
    ...
    handlebars
        .register_template_file("template", "resource/handlebars/java/java.hbs")
        .unwrap();
